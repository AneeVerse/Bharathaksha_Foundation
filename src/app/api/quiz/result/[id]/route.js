...
export async function GET(req, { params }) {
  try {
    ...
    // If the requester is a superadmin, allow access to any quiz result; otherwise, restrict to own
    const query = { _id: id };
    if ((decodedToken?.role || 'user') !== 'superadmin') {
      query.userId = decodedToken.userId;
    }

    // Fetch the quiz result
    const quizResult = await QuizResultModel.findOne(query);
