              ...
              {menuList.map((val, ind) => (
                <div key={ind} className="relative group">
                  {val.url ? (
                    <Link
                      href={val.url}
                      className={`hover:text-[#8ac240] font-semibold ${
                        val.url === pathName ? "text-[#8ac240]" : "text-black"
                      }`}
                    >
                      {val.name}
                    </Link>
                    ...
                    <div className="absolute left-0 min-w-max py-2 hidden group-hover:block bg-white shadow-lg rounded-md">
                      {val.subLink.map((sub, subInd) => (
                        <Link
                          key={subInd}
                          href={sub.url}
                          className={`block px-4 py-2 hover:bg-gray-100 hover:text-[#8ac240] ${
                            sub.url === pathName
                              ? "bg-gray-100 text-[#8ac240]"
                              : "text-black"
                          }`}
                        >
                          {sub.name}
              ...
              {!isLoggedIn && (
                <>
                  <Link
                    href="/auth/login"
                    className={`hover:text-[#8ac240] font-semibold ${
                      pathName === "/login" ? "text-[#8ac240]" : "text-black"
                    }`}
                  >
                    Sign in
                  </Link>
            ...
            <div key={ind} className="">
              {val.url ? (
                <Link
                  href={val.url}
                  onClick={() => setIsOpen(!isOpen)}
                  className={`block text-gray-900 hover:bg-gray-700 ${
                    pathName === val.url ? "text-white bg-gray-700" : ""
          ...
          {!isLoggedIn ? (
            <div className="mt-2">
              ...
              <Link
                ...
                onClick={() => setIsOpen(!isOpen)}
                className={`block text-gray-900 hover:bg-gray-700 ${
                  pathName === "/login" ? "text-white bg-gray-700" : ""
                } hover:text-white px-3 py-2 rounded-md text-base font-medium`}
              >
                Sign in
              </Link>
